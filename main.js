...
class Puzzle {
  constructor(el, width = 24, height = 24, row = 2, col = 2, gap = 1) {
    this.el = el;
  ...
  constructor(el, total) {
    ...
    this.draggie = new Draggable(el, {
      ...
      onRelease: function () {
        ...
        const orders = Array.from(that.total).map((item) =>
          item.style.getPropertyValue("--order")
        );
        const ids = Array.from(that.total).map((item) =>
          item.style.getPropertyValue("--i")
        );
...
const start = () => {
  ...
  const sortables = Array.from(fragments).map(
    (item) => new Sortable(item, fragments)
  );
};
